...
<div class="form-group">
    ...
    <input type="time" class="form-control  @error('horaInicio') is-invalid @enderror" name="horaInicio" id="horaInicio"
...
<div class="form-group">
    ...
    <input type="time" class="form-control  @error('horaFinaliza') is-invalid @enderror" name="horaFinaliza"
        id="horaFinaliza" value="{{old('horaInico', $evento->horaFinaliza)}}">
    @error('horaFinaliza')
    <span class="invalid-feedback" role="alert">
        <strong>{{$message}}</strong>
    </span>
...
<div class="form-group">
    ...
    <input type="number" class="form-control  @error('cantidad') is-invalid @enderror" name="cantidad" id="cantidad"
        value="{{old('horaInico', $evento->cantidad)}}">
    @error('cantidad')
    <span class="invalid-feedback" role="alert">
        <strong>{{$message}}</strong>
    </span>
...
<div class="form-check form-check-inline d-flex flex-column align-items-start ">
    ...
    <div>
        ...
        {{-- {{dd($arboles)}} --}}
        <input class="form-check-input " type="checkbox" value="{{$id}}" {{$evento->arboles->pluck('id')->contains($id)
        ? 'checked' : ''}}
        name="arboles[]">
        <label class="form-check-label pr-2 @error('arboles') is-invalid @else border-0 @enderror">{{$nombrePopular}}
        </label>
        @endforeach
        @error('arboles')
        <span class="invalid-feedback" role="alert">
